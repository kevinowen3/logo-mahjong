...
- **Win modal** (`#winModal`) renders five post-clear data points stacked between "You cleared all the tiles!" and the Play Again button: a gold-trophy SVG (`.win-trophy`), a level label (`Level: <span id="winLevel">`), the elapsed time (`<p class="win-time" id="winTime">Finished in <strong>—</strong></p>`), the shuffle count (`<strong id="winShuffles">`), and the hint count (`<strong id="winHints">`). All four text lines have uniform `0.5rem 0` margins so margin-collapsing produces equal visible gaps; `.win-hints` (the last line) keeps a 1.2rem bottom for breathing room before the button. `gameStartMs` uses **first-click-start**: `newGame()` sets it to `null` (sentinel) and the first time `onTileClick()` runs past the blocked-tile guard, it stamps `gameStartMs = Date.now()`. Pre-game thinking time is therefore not counted, but every shuffle and second of mid-game idle still is — `doShuffle()` deliberately does not reset `gameStartMs`, so a shuffled game stays in the same timed session and its full clearing time counts toward the player's personal best. `doShuffle()` increments `shuffleCount`; `doHint()` increments `hintCount` only on the success branch (skipped when "No matching pairs available" — that path doesn't reveal a hint, so we don't count it). `newGame()` resets both to 0 so each game's counts are independent. The stamp happens after the blocked-tile guard but before the hint-accept / selection branches, so any meaningful first interaction starts the clock; clicking a blocked tile does not.
- **Win-modal animation system**: a single `showWinModal({ levelLabel, elapsedMs, shuffles })` helper drives every show path (real win in `checkEndConditions()`, Shift+Click test harness on `#gameTitle`). It coordinates four effects, all gated on `#winModal:not(.hidden)` so they re-fire each time the modal opens:
  1. **Springy CSS entrance** — `.modal-content` runs `winModalIn` with `cubic-bezier(0.34, 1.56, 0.64, 1)` overshoot for ~450 ms; the children (`h2`, `<p>` lines, button) stagger-fade-in via `winLineIn` at 100 ms / 200 ms / 300 ms / 400 ms / 500 ms / 600 ms `animation-delay` increments.
...
**Audio** is synthesized in pure JavaScript via the Web Audio API — no asset files, fits the no-build philosophy. Three sounds, all gated on `readSfxEnabled()` (persisted in `localStorage['logo:sfx-enabled']`, default ON, safe-fallback in private-browsing modes that throw on storage access):
...
- **`playWinFanfare()`** — ~2.5 s triumphant sequence: rising C-major pentatonic arpeggio (C5 / E5 / G5 / C6 / E6 / G6 at 100 ms intervals) cascading into a sustained C-major chord (C4 / E4 / G4 / C5), with a soft C3 sine bass underneath. Each "bell" note stacks fundamental + 2nd + 3rd harmonic sines (decreasing amplitude) so the timbre reads warm-bell rather than sterile-sine — the same trick celesta/glockenspiel sample libraries use. Pentatonic-rising can't form a dissonant interval, and the major-chord resolution is the "you did it!" beat. Fires from the win path of `checkEndConditions()` and from the Shift+Click test harness on `#gameTitle`. Earlier prototypes (filtered-noise applause; voice-fake "wooo") sounded harsh / sterile in pure synthesis; the bell fanfare is the keeper — don't go back to noise-burst applause.
