...
- **Win modal** (`#winModal`) renders four post-clear data points stacked between "You cleared all the tiles!" and the Play Again button: a gold-trophy SVG (`.win-trophy`), a level label (`Level: <span id="winLevel">`), the elapsed time (`<p class="win-time" id="winTime">Finished in <strong>—</strong></p>`), and the shuffle count (`<strong id="winShuffles">`). All three text lines have uniform `0.5rem 0` margins so margin-collapsing produces equal visible gaps; `.win-shuffles` keeps a 1.2rem bottom for breathing room before the button. `gameStartMs` uses **first-click-start**: `newGame()` sets it to `null` (sentinel) and the first time `onTileClick()` runs past the blocked-tile guard, it stamps `gameStartMs = Date.now()`. Pre-game thinking time is therefore not counted, but every shuffle and second of mid-game idle still is — `doShuffle()` deliberately does not reset `gameStartMs`, so a shuffled game stays in the same timed session and its full clearing time counts toward the player's personal best. `doShuffle()` increments `shuffleCount`; `newGame()` resets it to 0 so each game's count is independent. The stamp happens after the blocked-tile guard but before the hint-accept / selection branches, so any meaningful first interaction starts the clock; clicking a blocked tile does not.
- **Win-modal animation system**: a single `showWinModal({ levelLabel, elapsedMs, shuffles })` helper drives every show path (real win in `checkEndConditions()`, Shift+Click test harness on `#gameTitle`). It coordinates four effects, all gated on `#winModal:not(.hidden)` so they re-fire each time the modal opens:
  ...
  3. **Count-up** — `animateCount(el, fromVal, toVal, durationMs, formatter, onFrameId)` uses `requestAnimationFrame` + `easeOutCubic`. Time animates 0 → final over 800 ms via `formatDuration`; shuffles 0 → final over 600 ms via `String(Math.round(v))`. Counters start ~300 ms after modal show via `countStartTimeoutId` so the entrance reads first. `cancelWinAnimations()` cancels both rAF handles + the start timeout — called at the top of every `showWinModal()` and from `hideWinModal()` (which wraps `winModal.classList.add('hidden')` plus cleanup, and is now called from `newGame()` and the Escape handler in place of the bare `add('hidden')` so confetti/counters don't leak across opens).
...
- **Test harness for the win modal**: Shift+Click on `#gameTitle` calls `showWinModal` with `randomReasonableWinMs(currentDifficulty)` and `randomReasonableShuffles(currentDifficulty)` — the harness exercises the full animation path (entrance, trophy, count-up, confetti) end-to-end. `WIN_TIME_TEST_RANGES_MS` defines plausible per-level time ranges (Newb 1.5–4 min, Padawan 3–8 min, Smarty Pants 8–20 min); `WIN_SHUFFLES_TEST_MAX` caps shuffles at 2 / 3 / 5 with an `r*r` skew toward 0 so the harness mostly shows "Shuffles: 0" matching real play. The harness deliberately bypasses `recordWinTime` / `bumpWins` (so it doesn't pollute personal bests or shared counters).
